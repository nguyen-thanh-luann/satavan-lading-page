...
export function Star({
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLSpanElement>) => {
    ...
    const { left, right, width }: any = currentTarget.children[0]?.getBoundingClientRect()
          ...
          {[...Array(iconsCount)].map((_, index) => (
            <Fragment key={index}>
              {customIcons[index]?.icon || emptyIcon || (
                <StarIcon key={index} size={size} className="ml-8 first:ml-0" />
              )}
            </Fragment>
          ))}
        </span>
          ...
          {[...Array(iconsCount)].map((_, index) => (
            <Fragment key={index}>
              {customIcons[index]?.icon || fullIcon || (
                <StarIcon size={size} className="ml-8 first:ml-0" />
              )}
            </Fragment>
          ))}
        </span>
